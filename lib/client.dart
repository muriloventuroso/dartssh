...
class SSHClient extends SSHTransport with SSHAgentForwarding {
  ...
  void handleMSG_KEXDH_REPLY(int packetId, Uint8List packet) async {
    if (state != SSHTransportState.FIRST_KEXREPLY &&
    ...
    if (packetId == MSG_KEX_ECDH_REPLY.ID &&
      ...
      fingerprint = await handleDhMSG_KEXDH_REPLY(
              MSG_KEXDH_REPLY()..deserialize(packetS!)) ??
          fingerprint;
    }
  ...
  Future<Uint8List?> handleX25519MSG_KEX_ECDH_REPLY(
      MSG_KEX_ECDH_REPLY msg) async {
    Uint8List? fingerprint;
    if (tracePrint != null) {
    ...
    if (!(await computeExchangeHashAndVerifyHostKey(msg.kS!, msg.hSig))) {
  ...
  Future<Uint8List?> handleEcDhMSG_KEX_ECDH_REPLY(
      MSG_KEX_ECDH_REPLY msg) async {
    Uint8List? fingerprint;
    if (tracePrint != null) {
  ...
  Future<Uint8List?> handleDhMSG_KEXDH_REPLY(MSG_KEXDH_REPLY msg) async {
    Uint8List? fingerprint;
    if (tracePrint != null) {
  ...
  void handleMSG_USERAUTH_INFO_REQUEST(MSG_USERAUTH_INFO_REQUEST msg) {
    ...
    if (msg.prompts.isNotEmpty) {
      ...
      writeCipher(MSG_USERAUTH_INFO_RESPONSE([]));
    }
  }
  ...
  Future<bool> computeExchangeHashAndVerifyHostKey(
      Uint8List kS, Uint8List? hSig) {
  ...
  Future sendAuthenticationRequest() async {
    if (identity == null) {
      if (debugPrint != null) {
      ...
      Uint8List pubkey = (await identity!.getEd25519PublicKey()).toRaw();
      Uint8List challenge = deriveChallenge(sessionId!, login, 'ssh-connection',
          'publickey', 'ssh-ed25519', pubkey);
      Ed25519Signature sig = await identity!.signWithEd25519Key(challenge);
  ...
  /// Sends env [MSG_CHANNEL_REQUEST].
  void setEnvVariable(String name, String value) {
    if (socket == null || sessionChannel == null) return;

    writeCipher(MSG_CHANNEL_REQUEST.env(sessionChannel!.remoteId, 'env', false,
        utf8.encode(name) as Uint8List?, utf8.encode(value) as Uint8List?));
  }
...
class SSHTunneledSocketImpl extends SocketInterface {
  ...
  SSHTunneledSocketImpl(Uri url, String login, Identity key, String password,
      {StringCallback? print, StringCallback? debugPrint})
      : clientOwner = true {
    identity = key == null ? null : key;
    client = SSHClient(
        ...
        getPassword: password == null
            ? null
            : () => Future.value(Uint8List.fromList(utf8.encode(password))),
